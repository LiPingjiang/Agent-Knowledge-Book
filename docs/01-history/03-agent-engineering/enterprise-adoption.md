...
## 从"幻灭低谷"中学到的：项目失败的系统性复盘

2023-2025 年的行业数据描绘了一幅严峻的图景。Gartner 预测到 2027 年将有 40% 旨在实现通用自主应用的 AI Agent 项目被叫停。麦肯锡对 50 个已实施 Agent 项目的研究显示，超过 70% 未达预期目标，其中近 40% 被认定为彻底失败。爱分析对央国企 Agent 落地的调研也证实了类似趋势。这些失败并非源于单一技术瓶颈，而是多重因素交织作用的结果。

通过综合分析行业报告和案例复盘，项目失败的根因可归纳为六个维度：

**场景选错与价值错位（约 90% 失败项目可追溯至此）**——高达九成的失败项目可追溯至"为了 AI 而 AI"的战略迷航。典型表现是选择过于复杂的长流程任务（如完全替代医生的远程问诊 Agent），或设定不切实际的目标而缺乏明确的价值衡量指标。某医疗集团 2024 年启动的全流程问诊 Agent 项目，因无法保证诊断准确性和安全性，于 2025 年底被迫终止。

**数据地基不牢（占比约 25%）**——数据质量低下、数据孤岛和知识库构建不足共同构成了"沙上建塔"的困境。企业 Agent 需要的数据散落在 ERP、CRM、SCM 等不同系统中，打通成本远超预期。特别是在央国企场景中，数据与知识质量问题被列为首要失败原因。

**工程落地系统性疏忽**——麦肯锡总结了六大工程实践缺陷：未将 Agent 嵌入端到端流程（孤立开发）、误认为 Agent 是万能解药、缺乏评估与迭代体系、忽视"人在环路"的关键审核节点、缺乏模块化可复用设计、技术与业务团队协作不畅。典型技术踩坑包括路径规划错误、复杂格式处理失败、上下文窗口溢出和 API 误用。

**成本失控与 ROI 不达标**——企业普遍只看到模型 API 调用成本，严重低估了数据准备、系统集成、持续维护、合规审计以及"人在环路"所需的人力成本。一个在 PoC 中表现优异的银行风控 Agent，部署到生产环境后因真实数据噪声导致性能急剧下降，需要数倍成本优化。

**人才断层与组织协同障碍**——73% 的企业认为缺乏所需人才是推动 AI 项目的关键瓶颈。市场极度稀缺既懂业务逻辑又懂 AI 技术的复合型人才。同时，组织僵化、部门墙高耸使得 Agent 所需的流程重构阻力重重。

**市场泡沫与非理性竞争**——大量初创公司涌现但产品同质化严重，"智能体包装"现象普遍（将传统 RPA 或 Chatbot 包装为"AI Agent"销售），基础模型和框架迭代过快增加了技术选型风险。

这些教训将在 [经验教训](../04-lessons-learned/reliability-engineering.md) 章节中进一步展开，此处仅作为企业采纳图景的重要背景。核心启示是：Agent 的成功不取决于模型"够不够聪明"，而取决于将智能转化为可靠、可控、有价值的生产力工具所需的系统工程能力。
...
- [McKinsey, 2025] "AI Agent 项目为什么会失败？50 个项目实践的六个启示"
- [Gartner, 2025] "Market Guide for AI Agent Platforms"
- [Gartner, 2025] "40% of AI Agent Projects Will Be Abandoned" — Agent 项目失败率预测
- [爱分析, 2026] "央国企 Agent 落地进展研究报告" — 中国市场 Agent 采纳数据
...
- 本书 [Multi-Agent 生产部署实证](../../02-technology/08-multi-agent/production-deployment.md) 章节
