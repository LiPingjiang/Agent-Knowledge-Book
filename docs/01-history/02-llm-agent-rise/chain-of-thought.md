...
2022 年 1 月，Google Brain 的 Jason Wei 等人提交了论文"Chain-of-Thought Prompting Elicits Reasoning in Large Language Models" [Wei et al., 2022]。这篇论文的核心发现简洁得令人惊讶：仅仅通过在少样本提示（[Few-shot](../../appendix/glossary.md#few-zero-shot) Prompt）中展示推理的中间步骤，就能大幅提升模型在复杂推理任务上的表现。
...
结果令人震惊：在 GSM8K 数学推理基准上，标准提示的 PaLM-540B 准确率仅为 17.9%，而加入思维链后飙升至 58.1%。更重要的是，这种提升在小模型上几乎不存在——只有参数量超过约 1000 亿的模型才表现出 CoT 带来的显著增益。这暗示推理能力是一种**[涌现能力](../../appendix/glossary.md#emergent-abilities)**（Emergent Ability），只在模型规模跨过某个临界点后才会出现。
...
这就是[零样本](../../appendix/glossary.md#few-zero-shot)思维链（Zero-shot CoT）。一句看似平淡无奇的提示语，却能将模型在 MultiArith 数学基准上的准确率从 17.7% 提升到 78.7%。这个结果的冲击力在于：它暗示推理能力并非通过示例"注入"到模型中的，而是已经存在于模型内部——我们只是需要找到正确的"钥匙"来激活它。
...
CoT 的一个问题是：单次推理可能出错。模型可能在某一步做出错误的推导，导致最终答案错误。Wang 等人提出了自一致性（Self-Consistency）方法 [Wang et al., 2022]：对同一问题生成多条推理路径（通过设置较高的 [Temperature](../../appendix/glossary.md#temperature)），然后通过多数投票选择最终答案。
...
**过程奖励模型（Process Reward Model, [PRM](../../appendix/glossary.md#prm-orm)）**：2023 年 5 月，OpenAI 发表了"Let's Verify Step by Step" [Lightman et al., 2023]，提出不仅奖励最终答案的正确性，还对每一步推理的正确性进行评估和奖励。这解决了一个关键问题：传统的结果奖励模型（[ORM](../../appendix/glossary.md#prm-orm)）无法区分"过程正确但结论错误"和"过程错误但碰巧正确"的情况。对 Agent 而言，PRM 意味着我们可以在推理过程中实时检测错误，而不是等到最终结果出来才发现问题。

**推理与搜索的融合**：将推理过程建模为搜索问题，使用[蒙特卡洛树搜索（MCTS）](../../appendix/glossary.md#mcts)等方法来优化推理路径选择。这种方法在 AlphaGo 中取得了巨大成功，而将其应用于语言推理则代表了一种将经典 AI 智慧与现代 LLM 结合的趋势。
...
阶段 2: 面向推理的强化学习
  - 使用 [GRPO](../../appendix/glossary.md#grpo)（Group Relative Policy Optimization）算法
...
**GRPO 的核心思想：** 传统 RL（[PPO](../../appendix/glossary.md#ppo)）需要一个单独的 Critic 模型估计价值函数。[GRPO](../../appendix/glossary.md#grpo) 简化了这一点：对同一个问题采样一组回答（如 16 条），用组内的相对排序作为奖励信号。答案正确的推理链获得正奖励，错误的获得负奖励。这避免了训练额外的 Critic 模型，大幅降低了训练成本。
...
2024-2025 年的[机械可解释性](../../appendix/glossary.md#mechanistic-interpretability)（Mechanistic Interpretability）研究揭示了答案：思维链并不是让一整句话在空间中找到一个"逻辑坐标点"，而是通过三层嵌套的动态机制来实现推理。

#### 第一层：多维空间的叠加编码

当"有 3 个箱子，每个箱子有 2 个红球"进入模型时，它不是作为一个整体向量存在，而是在[残差流](../../appendix/glossary.md#residual-stream)（Residual Stream）的不同维度子空间中被分层编码：
...
具体到数字运算，Transformer 的[注意力头](../../appendix/glossary.md#attention-head)（Attention Head）充当了"数据搬运工 + 计算器"的角色：

当模型在思维链中写下"总共有 3 × 2 = 6 个红球"时，向量空间中发生的操作是：

1. **提取**：专门的注意力头同时"注视"（attend to）"3"和"2"的 Token 位置，将它们的数量维度特征提取出来
2. **运算**：这些特征通过 [MLP](../../appendix/glossary.md#mlp) 层的权重矩阵进行非线性变换——在几何上表现为特定方向的投影和平移
