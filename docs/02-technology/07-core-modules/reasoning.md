...
flowchart TB
    ...
    subgraph LOOP["解码循环（推理引擎，非模型本身）"]
        direction TB
        M["LLM 神经网络<br/>输出 logits（整个词表的分数）"] --> S["状态机查询：当前能接哪些 token?"]
        S --> MASK["把非法 token 的 logit 设为 -∞（mask）"]
        MASK --> SAMPLE["采样 → 选出下一个 token"]
    end

    NOTE["← 约束就在这两步！<br/>状态机查询 + logit mask"] -.-> S
    SAMPLE --> APPEND["追加到序列"]
    APPEND -- "未结束，继续下一个 token" --> M
    APPEND -- "生成结束" --> DONE["输出完整回答"]

    style LOOP fill:#fff3e0,stroke:#f57c00,stroke-width:2px
    style M fill:#e8f5e9,stroke:#388e3c
    style S fill:#ffe0b2,stroke:#f57c00
    style MASK fill:#ffccbc,stroke:#e64a19
    style NOTE fill:#fff,stroke:#e64a19,stroke-dasharray:4
```

绿色的「LLM 神经网络」只负责产出 logits，橙色的「状态机查询 + logit mask」才是约束施加的地方——它由推理引擎插在模型出 logits 与采样之间，模型权重完全不变。

一句话概括：**模型负责"想说什么"，解码循环负责"不许说违规的"。**
...
两者是严格嵌套的关系（注意图中有**两个回边、两个循环**：内层 `继续下一个 token` 是解码循环，外层 `第 N+1 轮` 才是 Agent 循环）：

```mermaid
flowchart TB
    subgraph AGENT["Agent 循环 — 你的应用进程 · 单位:一轮 think-act"]
        direction TB
        A1["第 N 轮: 推理 / 决策"] --> A2["发起 1 次 LLM 调用"]
        A2 -. "网络请求" .-> LLMCALL
        A6["拿到结果 → 执行工具 → 观察"] --> A7["第 N+1 轮 ..."]
    end

    subgraph LLMCALL["一次 LLM 调用 — 推理服务 · OpenAI / 自部署 vLLM"]
        direction TB
        subgraph DECODE["解码循环 — 单位:1 个 token · 反复迭代成百上千次"]
            direction TB
            D1["LLM 神经网络<br/>输出 logits（整个词表的分数）"] --> D2{"是否约束解码?<br/>response_format / grammar"}
            D2 -- "是" --> D3["状态机算出合法 token<br/>非法 token 的 logit = -∞（mask）"]
            D2 -- "否" --> D4["不做 mask"]
            D3 --> D5["采样 → 选出 1 个 token"]
            D4 --> D5
            D5 --> D6{"生成结束?<br/>EOS / 达到 schema 终态"}
            D6 -- "否，继续下一个 token" --> D1
        end
        D6 -- "是，组装完整回答（如 tool_use 块）" --> RET["返回结果"]
    end

    RET -. "网络返回" .-> A6

    style AGENT fill:#e3f2fd,stroke:#1976d2,stroke-width:2px
    style LLMCALL fill:#e8f5e9,stroke:#388e3c,stroke-width:2px
    style DECODE fill:#fff3e0,stroke:#f57c00,stroke-width:2px
    style D2 fill:#ffe0b2
    style D3 fill:#ffccbc
```

图中三层颜色由外到内：蓝色是 **Agent 循环**（你写的编排，跑在应用进程），绿色是 **一次 LLM 调用**（一次网络请求触发，跑在推理服务），橙色是 **解码循环**（藏在 LLM 调用内部，单位是一个 token）。**约束解码的 logit mask 就发生在最内层橙色框的每一次迭代里**——它既不是 Agent 的步骤，也不是模型权重的一部分，而是推理引擎在"模型出 logits"和"采样"之间插入的一道工序。
