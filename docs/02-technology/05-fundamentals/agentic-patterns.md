...
│  基础模式     │    进阶模式       │   自主模式     │   协作模式     │
│              │                  │              │              │
│ Prompt       │ Orchestrator-    │ ReAct Loop   │ Debate /     │
│  Chaining    │  Worker          │              │  Discussion  │
│              │                  │              │              │
│ Routing      │ Evaluator-       │ Plan-and-    │ Role-Playing │
│              │  Optimizer       │  Execute     │              │
│              │                  │              │              │
│ Parallel-    │                  │ Reflexion    │ Handoff      │
│  ization     │                  │              │              │
├──────────────┴──────────────────┴───────────────┴───────────────┤
│ Augmented LLM (基础构建块: LLM + Retrieval + Tools + Memory)      │
└─────────────────────────────────────────────────────────────────┘
```
...
```
Input → [Classifier] → Type A → [Handler A (specialized prompt/model)]
                      → Type B → [Handler B (specialized prompt/model)]
                      → Type C → [Handler C (specialized prompt/model)]
```
...
```
Sectioning:                          Voting:
Input → [Task A] ──┐                Input → [Attempt 1] ──┐
      → [Task B] ──┼→ Aggregator         → [Attempt 2] ──┼→ Majority Vote / Best-of-N
      → [Task C] ──┘                     → [Attempt 3] ──┘
```
...
Input → [Orchestrator LLM]
              ...
              │
              ├── [Worker 1: 子任务 A] ──┐
              ├── [Worker 2: 子任务 B] ──┼── [Orchestrator: 综合结果] → Output
              └── [Worker 3: 子任务 C] ──┘
```
...
```
[Generator] → Output v1 → [Evaluator] → "不满足标准，具体问题是..." 
     ↑                                         │
     └─────── 基于反馈修改 ←──────────────────────┘
...
```
Phase 1 - Planning:
Input → [Planner LLM] → Step1, Step2, Step3, ...

Phase 2 - Execution:
for step in plan:
    result = [Executor LLM/Tool](step)
    if need_replan(result):
        plan = [Planner LLM].replan(original_plan, results_so_far)
```
...
```
Triage Agent → 判断用户意图 → handoff(Billing Agent)
...
```
你的任务能用单次 LLM 调用解决吗？
├── 是 → 优化 Prompt + Retrieval，不需要 Agentic 模式
└── 否 → 子任务是固定的还是动态的？
         ├── 固定 → 子任务之间有依赖吗？
         │         ├── 有（线性依赖）→ Prompt Chaining
         │         └── 无（独立）→ Parallelization
         └── 动态 → 需要迭代优化吗？
                   ├── 是 → 有明确评估标准吗？
