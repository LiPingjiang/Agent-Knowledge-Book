...
**MCTS**（Monte Carlo Tree Search，蒙特卡洛树搜索）是一种通过随机模拟来指导搜索决策的算法。四个阶段循环：选择（Selection）→ 扩展（Expansion）→ 模拟（Simulation/Rollout）→ 反向传播（Backpropagation）。AlphaGo 使用 MCTS + 深度神经网络，在围棋中击败人类冠军。在 Agent 规划中，MCTS 被用于在多个推理路径间进行结构化探索。详见 [规划模块](../02-technology/07-core-modules/planning.md) 中 LATS 等应用。
...
**Token** 是大语言模型处理文本的基本单位。它不等于"字"也不等于"词"，而是由分词器（Tokenizer）根据统计规律将文本切割成的片段。对英文而言，一个 token 通常是 3-4 个字母（如 "running" 可能被切为 "run" + "ning"）；对中文而言，常见汉字通常 1 个字 = 1-2 个 token（取决于使用哪种分词器，如 GPT-4 的 cl100k_base 中大多数常用汉字是 1 个 token，而生僻字可能是 2-3 个 token）。粗略估算：1K token ≈ 750 个英文单词 ≈ 500-600 个中文字。模型的上下文窗口、计费、生成速度都以 token 为单位衡量。

### Prompt {#prompt}

**Prompt**（提示/提示词）是用户或系统提供给大语言模型的输入文本，用于引导模型生成期望的输出。它包含几个层次：**System Prompt**（系统提示）定义模型的角色、行为规则和全局约束；**User Prompt**（用户提示）是当前用户的具体请求；**Few-shot examples**（少样本示例）通过示例展示期望的输入-输出格式。Prompt Engineering（提示工程）是设计和优化 Prompt 以获得更好模型表现的实践。Prompt 是上下文窗口中的"输入部分"——上下文窗口 = Prompt + 模型生成的输出。

### 上下文窗口 {#context-window}

**上下文窗口**（Context Window）是 Transformer 模型一次前向计算中能处理的 token 总数上限。它是自注意力机制的物理边界——在标准 Transformer 中，每个 token 需要与窗口内所有其他 token 计算注意力分数，计算复杂度为 O(N²)。GPT-4 Turbo 为 128K token，Claude 3 为 200K token，Gemini 1.5 Pro 为 1M+ token。上下文窗口之外的信息对模型完全不可见——这不是"记忆模糊"而是"彻底不存在"。解决窗口限制的工程手段包括：滑动窗口截断、摘要压缩、RAG 检索增强、以及外部记忆系统。

### 自回归生成 {#autoregressive-generation}

**自回归生成**（Autoregressive Generation）是当前主流大语言模型（GPT、Claude、Llama 等）的文本生成范式。核心思想：将联合概率 P(x₁, x₂, ..., xₙ) 分解为条件概率的连乘 P(x₁)·P(x₂|x₁)·P(x₃|x₁,x₂)·...·P(xₙ|x₁,...,xₙ₋₁)。每一步生成一个 token，以所有之前已生成的 token 作为条件。这意味着生成是严格从左到右的单向过程——已输出的 token 不可修改（除非重新生成整个序列）。自回归的优势在于训练简单（next-token prediction 可大规模并行）且生成质量高（每步都有完整上下文条件），劣势在于推理速度慢（必须逐 token 串行）且无法原生回溯。

### 世界模型 {#world-model}

**世界模型**（World Model）是一个能预测环境状态转移的内部表征系统——给定当前状态 s 和动作 a，输出预测的下一状态 s' 和奖励 r。这个概念跨越多个领域：在认知科学中指人类大脑对外部世界的心理表征（Mental Model），支撑"心理模拟"能力；在强化学习中指一个可学习的转移模型 M(s,a)→(s',r)，如 Dreamer 和 MuZero 中的神经网络；在 LLM 研究中则是一个争议话题——有证据表明 Transformer 可能形成了隐式的世界表示（Othello-GPT, Li et al., 2023），但远不如显式模型可靠和可控。世界模型的核心价值在于支持"想象中的规划"——在不实际执行动作的情况下预见后果，从而安全高效地搜索最优策略。
...
| P | [PDDL](#pddl), [POMDP](#pomdp), [PPO](#ppo), [Prompt](#prompt), [PSPACE-complete](#pspace-complete), [Pareto optimal](#pareto-optimal), [Policy Gradient](#policy-gradient), [Pub-Sub](#pub-sub), [产生式规则](#production-rules) |
...
| T | [Target Network](#target-network), [Temperature](#temperature), [Token](#token), [Trace/Span](#trace-span), [Transformer](#transformer), [拓扑排序](#topological-sort) |
| V | [Vickrey 拍卖](#vickrey-auction), [Voronoi](#voronoi) |
| W | [世界模型](#world-model) |
| 其他 | [2-EXPTIME](#2-exptime), [Data Contamination](#data-contamination), [Chunked Encoding](#chunked-encoding), [上下文窗口](#context-window), [自回归生成](#autoregressive-generation), [框架问题](#frame-problem), [物理符号系统假说](#physical-symbol-system), [手段-目的分析](#means-ends-analysis), [情景记忆](#episodic-memory), [组合拍卖](#combinatorial-auction), [组合爆炸](#combinatorial-explosion) |
